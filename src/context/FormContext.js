...
export const FormProvider = ({ children }) => {
    ...
    const canNextPage1 = Object.keys(data)
        .filter(key => key.startsWith('car') && key !== 'billAddress2')
        .map(key => data[key])
        .every(Boolean)
    ...
    const nextHide = page === Object.keys(title).length - 2 && "remove-button"

    const submitHide = page !== Object.keys(title).length - 2 && "remove-button"
